...
pg_dump -a $PROD -h $PGHOST -U $PGUSER --disable-triggers -t 'cognicity.*' -t 'detik.*' -t 'floodgauge.*' -t 'grasp.*' -t 'infrastructure.*' -t 'qlue.*' -t 'sensors.*' -t 'twitter.*' | psql -d $NEW -h $PGHOST -U $PGUSER
